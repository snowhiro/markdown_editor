...
* **状態の保持**: 分割表示のON/OFFと左右の幅比はアプリ実行中は保持し、他モードからEditモードへ戻った際に前回の状態を復元する。アプリ再起動をまたぐ永続化は本項のスコープ外とする（12章）
...
* **スクロール連動**:
  ...
  * ソース行とレンダリング結果の行単位での正確な対応付け（markdown-itのソース行情報を用いたマッピング）は本項のスコープ外とし、将来検討とする（12章）
